...
enum ChartDataDateRange {
    case today
    case todayAndMostRecentDay
    case lastSevenDays
    case thisMonth
}


typealias FeedSessionChartDataOutput = (data: [Date: [FeedSession]], min: Int32, max: Int32, reports: FeedSessionChartDataReport)

struct FeedSessionChartDataReport {
    var data: FeedSessionChartData
    var reportDataRange: ClosedRange<Date>
    
    init(data: FeedSessionChartData, reportDataRange: ClosedRange<Date>) {
        self.data = data
        self.reportDataRange = reportDataRange
    }
    
    func reportIntro() -> String {
        return "OK"
//        switch(self.reportDataRange) {
//            case .today: return self.dailyIntro()
//            case .todayAndMostRecentDay: return self.recentTwoDaysIntro()
//            case .lastSevenDays: return self.weeklyIntro()
//            case .thisMonth: return self.monthlyIntro()
//        }
    }
    ...
    func reportSummary() -> String {
        return "Test"
//        switch(self.reportDataRange) {
//            case .today: return self.dailySummary()
//            case .todayAndMostRecentDay: return self.recentTwoDaysSummary()
//            case .lastSevenDays: return self.weeklySummary()
//            case .thisMonth: return self.monthlySummary()
//        }
    }
    
    private func dailyIntro() -> String {
        print("getting dailyIntro\n\n")
        return "Daily Intro here \(arc4random())"
    }
    
    private func dailySummary() -> String {
        print("getting dailySymmary\n\n")
        return "Daily summary here \(arc4random())"
    }
    
    private func recentTwoDaysIntro() -> String {
        print("getting recentTwoDays intro\n\n")
        return "two days intro"
    }
    
    private func recentTwoDaysSummary() -> String {
        print("getting two day summary \n\n")
        return "two days summary"
    }
    
    private func weeklyIntro() -> String {
        print("getting weekly intro \n\n")
        return "weekly intro"
    }
    
    private func weeklySummary() -> String {
        print("getting symmary\n\n")
        return "weekly summary"
    }
    
    private func monthlyIntro() -> String {
        print("getting monthly intro \(arc4random())\n\n")
        return "monthly intro \(arc4random())"
    }
    
    private func monthlySummary() -> String {
        print("getting monthlysymmary\n\n")
...
class FeedSessionChartData: ObservableObject {
    ...
    func generateDataInRange(range: ClosedRange<Date>) -> FeedSessionChartDataOutput {
        let allSessions = self.child.completedFeedSessionsArray
        let sessions = allSessions.filter { range.contains( $0.wrappedCreatedAt )}
        ...
        if includeAllDatesInRange {
            feedSessionData = buildFeedSessionData(within: range, from: sessions)
        }else {
            // if we're not including all the dates in the range, just get the data for each of the bounds
            feedSessionData[range.lowerBound] = sessions.filter { $0.wrappedCreatedAt.beginningOfDay == range.lowerBound.beginningOfDay}
            feedSessionData[range.upperBound] = sessions.filter { $0.wrappedCreatedAt.beginningOfDay == range.upperBound.beginningOfDay}
        }
        ...
        let reporter = FeedSessionChartDataReport(data: self, reportDataRange: range)
        
        self.data = (data: feedSessionData, min: min, max: max, reports: reporter)
        return (data: feedSessionData, min: min, max: max, reports: reporter)
    }
...
//    private func chartDataDateRange(range: ChartDataDateRange, from sessions: [FeedSession]) -> ClosedRange<Date> {
//        if range == .today {
//            return Date().beginningOfDay...Date().endOfDay
//        }else if range == .todayAndMostRecentDay {
//            let earliestSessionBeforeToday = sessions.sorted {$0.wrappedCreatedAt > $1.wrappedCreatedAt}.first {
//                $0.wrappedCreatedAt < Date().beginningOfDay
//            }
//
//            let lower = earliestSessionBeforeToday?.wrappedCreatedAt.beginningOfDay ?? Date()
//
//            return lower...Date().endOfDay
//        }else if range == .lastSevenDays {
//            return Date().lastSevenDays...Date().endOfDay
//        }else { // .thisMonth
//            return Date().beginningOfMonth...Date().endOfDay
//        }
//    }
    ...
    private func buildFeedSessionData(within dateRange: ClosedRange<Date>, from sessions: [FeedSession]) -> [Date: [FeedSession]] {
        ...
        repeat {
            feedSessionData[nextDate] = sessions.filter {
                $0.wrappedCreatedAt.beginningOfDay == nextDate
            }
